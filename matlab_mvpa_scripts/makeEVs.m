%% Create Reg Arrays - ForceMemRI v1
...
%this analysis are Face, Scene, No-PM (OG only), and Rest.  This script
%will go through the OG localizer, the PM localizer, and the PM task.
clear
%Overall:
%Row 1 = Face
%Row 2 = Scene
...
for ogBlockI=1:ogBlocks
    ...
    for ogTrialI=1:ogTrBlock
        ...
        ogRegs(3,(ogBlockI-1)*ogTRs+startEV+1:(ogBlockI-1)*ogTRs+endEV)=1;
...
%ogMaster
cd ../mvpa_params
save('ogLocRegs.mat','ogRegs')
save('ogBlockRegs.mat','ogBlockR')
save('ogMasterArray.mat','ogMaster')
...
%purposes
pmTaskReg=zeros(4,trLen*blockN);
easyRegs=zeros(4,trLen*blockN);
...
for blockI=1:blockN
    ...
    for trialI=1:trialN
        ...
        pmTaskReg(pmType,(blockI-1)*trLen+curProbe+1:(blockI-1)*trLen+curProbe+2)=0;
        curProbe=curProbe+2;
        ...
        if probeL>7
            ...
            if pmType==3
                ...
                for probeI=1:probeL
                    ...
                    pmTaskReg(pmType,(blockI-1)*trLen+curProbe+probeI)=1;
                    ...
                    if Data.BlockTimings{blockI}.Trial{trialI}.probeTiming{probeI,2}<=8
                       %Add the Reg in the right row
                        easyRegs(pmType,(blockI-1)*trLen+curProbe+probeI)=1;                         
                    end
...
cd ../mvpa_params
save('pmTaskRegs.mat','pmTaskReg');
save('pmTaskBlockR.mat','pmBlockR');
save('pmTaskMaster.mat','pmMaster');
save('pmTaskEasyProbes','easyRegs');